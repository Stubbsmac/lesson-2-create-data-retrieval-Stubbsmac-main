...
SELECT author, title
FROM book
WHERE genre = 'Thriller' AND publication_year > 2000 AND pages > 400
...
SELECT *
FROM book
WHERE publication_year = 2008 AND publisher = 'Thomson Reuters'
ORDER BY price DESC;
...
--Your code below this line
SELECT author, publisher, pages
FROM book
WHERE (pages > 700 OR pages < 500) AND price < 35
ORDER BY pages DESC;
...
SELECT title
FROM video_game
WHERE platform != 'Mobile' AND metacritic_score > 85 AND metacritic_score < 100;
